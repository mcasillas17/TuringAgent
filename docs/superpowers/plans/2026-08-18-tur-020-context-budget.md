...
**Review amendments:** The independent review loop tightened the executed design beyond the initial examples below. The final implementation uses a `32768` default provider cap plus a validated `2048` output reservation, derives a smaller explicit Ollama `num_ctx` per request, selects OpenAI's output-limit field by model family, requires context limits and exact-wire estimation on the `Provider` interface, admits optional tool definitions and history with logarithmic exact re-estimation, searches recall once per run into a deduplicated bounded cache and re-ranks those hits using fetched message IDs plus live occurrence counts while converging against the admitted history suffix under a shared deadline and small pass budget, replaces an oversized tool-result body with an explicit omission marker while preserving its tool message and correlation ID, and emits a durable notice when a provider stops at the output limit. These amendments supersede the earlier `8192`/setter/no-output-reserve examples in this archived execution plan.
