...
`FILE` is the strictest built-in compatibility category. It protects generated source as well as wire and JSON compatibility, which matches the repository's checked-in Flutter and Go clients. Git exports the resolved base commit into an ephemeral directory, and Buf compares that exact schema without maintaining a second checked-in snapshot or cloning the worktree.
...
5. Resolve the fetched commit, export its `proto` directory with `git archive`, and run `buf breaking proto --against "<temporary-baseline>/proto"`. Reading the already-resolved local commit works for normal checkouts, linked worktrees, shallow clones, and non-default base branches.

The script emits distinct diagnostics for a missing tool, unsupported Buf version, invalid base ref, failed fetch, unresolved commit, failed baseline extraction, and schema incompatibility. It force-refreshes the remote-tracking ref so a legitimately rewritten base branch is compared at its current tip, and it never falls back to an older local baseline after a fetch failure.
