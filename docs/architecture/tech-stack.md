...
Recall search runs once per agent run and caches its hits. Before each dispatch, those hits are re-ranked against the budget-admitted request; the runtime permits up to three ranking/budget passes under one two-second deadline if adding recall changes the contiguous history suffix. One broad fallback allows possible duplication rather than silently excluding a fetched current-session turn from both paths.
