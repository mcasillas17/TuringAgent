...
Both context values must be integers from `1` through `16777216`. Each output value must be positive and smaller than its context window; invalid configuration stops the runtime at startup. Ollama receives `num_predict` plus an explicit `num_ctx` derived from exact admitted request bytes and output reserve, capped by `OLLAMA_CONTEXT_WINDOW_TOKENS`; a small request therefore does not force the cap's full KV-cache allocation. OpenAI-compatible providers and routed external agents use their window for local admission and receive `max_completion_tokens` for o1/o3/o4 model families or `max_tokens` otherwise, so no Ollama-only option leaks onto their wire format. Operators must match each cap to the selected model because Turing does not currently discover model capabilities.
...
Recall search runs once per agent run and caches one recall-budget-bounded payload per unique message plus lightweight per-term references. Before each dispatch, those hits are re-ranked against the budget-admitted request; the runtime permits up to three ranking/budget passes under one two-second deadline if adding recall changes the contiguous history suffix. One broad fallback allows possible duplication rather than silently excluding a fetched current-session turn from both paths.
