...
Recall search runs once per agent run and caches one recall-budget-bounded payload per unique message plus lightweight per-term references. Fetched history retains message IDs through context assembly, so re-ranking suppresses exact admitted rows; occurrence counts handle only live messages without IDs. The runtime permits up to three ranking/budget passes under one two-second deadline if adding recall changes the contiguous history suffix. One broad fallback allows possible duplication rather than silently excluding a fetched current-session turn from both paths.
