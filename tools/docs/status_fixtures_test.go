...
func TestStatusGuardRepositoryFixtures(t *testing.T) {
	...
	for _, test := range []struct{ name, path, old, replacement, claim string }{
		...
		{"runtime handshake removed", runtime + "mcp/client.go", `c.call(ctx, mcpwire.MethodInitialize,`, `c.call(ctx, "tools/list",`, "mcp-lifecycle"},
		{"registry handshake removed", orchestrator + "service/mcpregistry/client.go", `c.roundTrip(ctx, mcpwire.MethodInitialize,`, `c.roundTrip(ctx, "tools/list",`, "mcp-lifecycle"},
		{"bundled initialize removed", backend + "mcp-files/cmd/server/main.go", `case "initialize":`, `case "initialise":`, "mcp-lifecycle"},
		{"bundled version header check removed", backend + "mcp-system/cmd/server/lifecycle.go", "http.StatusBadRequest)", "http.StatusOK)", "mcp-lifecycle"},
		...
		{"GitHub acceptance disabled", orchestrator + "service/integrations/providers.go", "supported: true,", "supported: false,", "github-tools"},
		...
		{"unsupported connection refusal removed", orchestrator + "service/integrations/service.go", "if !entry.supported {", "if false {", "other-integration-tools"},
		{"credential provider filter removed", orchestrator + "repository/integrations.go", "AND provider = 'github'", "", "other-integration-tools"},
