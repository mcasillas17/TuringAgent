...
func featureStatusClaims() []statusClaim {
	...
	claims := []statusClaim{
		...
		{"flutter-search", true, []statusEvidence{
			...
			has(app+"lib/networking/api_client.dart", "Future<List<SearchHit>> searchMessages({required String query, int limit = 50,});"),
			...
			function(orchestrator+"service/sessions/service.go", "SearchMessages", "s.search.SearchMessageHits("),
			...
			function(orchestrator+"repository/sessions.go", "searchMessagesPredicate", "fts5Phrase(query)"),
			function(orchestrator+"repository/sessions_test.go", "TestSearchMessagesTreatsInputAsLiteralPhrase", "repo.SearchMessages("),
			function(orchestrator+"repository/search_hits.go", "SearchMessageHits", "r.searchMessageHits("),
			function(orchestrator+"repository/search_hits.go", "searchMessageHits", "searchMessagesPredicate("),
			function(orchestrator+"repository/search_hits_test.go", "TestSearchMessageHitsKeepLiteralPhraseInjectionResistance", "repo.SearchMessageHits("),
			...
			{path: app + "lib/features/search/search_screen.dart", require: "widget.onOpenSession(", absent: "Navigator."},
			...
			has(app+"test/ui/responsive_shell_backend_test.dart", "testWidgets('search selection stays open until dismissed',"),
		}},
		...
		{"explicit-cancel", true, []statusEvidence{
			has("proto/turing/v1/chat.proto", "rpc CancelRun("),
			has("proto/turing/v1/chat.proto", "rpc GetRunCancellation("),
			has(orchestrator+"app/app.go", "turingv1.RegisterChatServiceServer(publicServer, chatService)"),
			function(orchestrator+"service/chat/explicit_cancel.go", "CancelRun", "s.repo.CancelUserRun("),
			function(orchestrator+"repository/explicit_cancel.go", "CancelUserRun", "cancelRunTx("),
			function(orchestrator+"repository/explicit_cancel.go", "CancelUserRun", "runoutcome.UserCancellation()"),
			function(orchestrator+"repository/explicit_cancel.go", "CancelUserRun", "recordAuditTx("),
			function(orchestrator+"service/chat/explicit_cancel_test.go", "TestExplicitCancelPublicQueuedReplay", "h.chatClient.CancelRun("),
			function(orchestrator+"app/explicit_cancel_test.go", "TestExplicitCancelPublicAuthenticationAndInternalIsolation", "codes.Unauthenticated"),
			function(orchestrator+"app/explicit_cancel_test.go", "TestExplicitCancelAuthenticatedEndToEndLateOutputCannotRevive", "public.CancelRun("),
			function(orchestrator+"service/runtime/cancelled_terminal_exit_test.go", "TestCancelledNewerAttemptRejectsPredecessorTerminalExit", "after.ExecutionActive"),
			has(api, "await _chat.cancelRun("),
			has(api, "await _chat.getRunCancellation("),
			has(app+"lib/features/chat/chat_screen.dart", "RunStopControl("),
			has(app+"lib/features/chat/run_cancellation_controller.dart", "await api.cancelRun("),
			has(app+"test/features/chat_screen_test.dart", "testWidgets('pending Stop survives a tool-separated assistant bubble',"),
		}},
