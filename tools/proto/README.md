...
`generate.sh` resolves the global pub cache from `PUB_CACHE`; when unset it uses `$HOME/.pub-cache` on Unix-like systems and `%LOCALAPPDATA%\Pub\Cache` on Windows. It accepts the platform's extensionless, `.bat`, `.cmd`, or `.exe` `protoc-gen-dart` shim and verifies that the selected cache has `protoc_plugin` 25.1.0 globally activated. Unix-like systems pass that absolute executable directly to protoc. On Windows, the selected cache's `bin` directory is prepended to `PATH` for protoc's platform-aware lookup, which is required to launch Dart's `.bat` shim through `cmd.exe`. In both cases a different `protoc-gen-dart` elsewhere on `PATH` cannot win. Missing or mismatched installations fail with the exact `PUB_CACHE=... dart pub global activate protoc_plugin 25.1.0` repair command.
