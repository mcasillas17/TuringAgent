...
// TestApprovalResumeRequiresAnApprovalThisRunOwns closes the identity field
// that would otherwise be decorative. A resume that names an approval belonging
// to another run is a decision about something else.
func TestApprovalResumeRequiresAnApprovalThisRunOwns(t *testing.T) {
	repo := New(openTestDB(t))
	ctx := context.Background()
	enqueued, claimed, recovering := recoveringRun(t, repo, "worker-approval-identity")

	// A real approval, owned by a different run in a different session.
	other := enqueueRun(t, repo, "Other run")
	if err := repo.MarkRunRunning(ctx, other.RunID); err != nil {
		t.Fatal(err)
	}
	otherApproval, err := repo.CreateApproval(ctx, other.RunID, "", "general_assistant",
		"files.update", `{}`, "sha256:other", "2099-01-01T00:00:00Z")
	if err != nil {
		t.Fatal(err)
	}
	...
	if _, err := repo.ResumeRecoveringRun(ctx, ResumeRecoveringRunInput{
		RunID: enqueued.RunID, ExpectedStateVersion: recovering.StateVersion,
		WorkerID: "worker-approval-identity", AssignmentAttemptID: claimed.AssignmentAttemptID,
		ApprovalID: otherApproval.ApprovalID,
	}); !errors.Is(err, ErrRunTransitionConflict) {
		t.Fatalf("resume naming another run's approval = %v, want ErrRunTransitionConflict", err)
	}
	if _, err := repo.ResumeRecoveringRun(ctx, ResumeRecoveringRunInput{
		RunID: enqueued.RunID, ExpectedStateVersion: recovering.StateVersion,
		WorkerID: "worker-approval-identity", AssignmentAttemptID: claimed.AssignmentAttemptID,
		ApprovalID: "appr_never_existed",
	}); !errors.Is(err, ErrRunTransitionConflict) {
		t.Fatalf("resume naming an unknown approval = %v, want ErrRunTransitionConflict", err)
	}
	state, err := repo.GetRunState(ctx, enqueued.RunID)
	if err != nil {
		t.Fatal(err)
