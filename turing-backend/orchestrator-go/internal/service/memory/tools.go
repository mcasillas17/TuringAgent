...
var memoryTools = []memoryTool{
	{
		...
		schema: objectSchema([]string{"query"}, map[string]any{
			"query": map[string]any{
				"type":        "string",
				"description": "What to look for in accepted memory. At most 512 bytes of UTF-8.",
				"minLength":   1,
				"maxLength":   maxMemoryQueryBytes,
			},
		}),
	},
	{
		...
		schema: objectSchema([]string{"belief_id"}, map[string]any{
			"belief_id": map[string]any{
				"type":        "string",
				"description": "The stable id of an accepted memory, as returned by memory.search.",
				"minLength":   1,
				"maxLength":   maxBeliefIDBytes,
			},
		}),
	},
	{
		...
		schema: objectSchema([]string{"title", "body"}, map[string]any{
			"title": map[string]any{
				"type":        "string",
				"description": "A short name for the proposal, in the user's own terms. At most 200 characters.",
				"minLength":   1,
				"maxLength":   maxMemoryTitleRunes,
			},
			"body": map[string]any{
				"type":        "string",
				"description": "The complete claim being proposed. At most 16384 bytes of UTF-8; it is refused, never shortened.",
				"minLength":   1,
				"maxLength":   memoryfiles.MaxCandidateBodyBytes,
			},
			"kind": map[string]any{
				"type": "string",
				"enum": []any{
