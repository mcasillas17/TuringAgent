...
- **Generated proto code is committed.** After editing a `.proto`, use protoc 34.1, protoc-gen-go 1.36.11, protoc-gen-go-grpc 1.6.2, and globally activated Dart `protoc_plugin` 22.5.0. Generation invokes the exact `protoc-gen-dart` under `PUB_CACHE` (default `$HOME/.pub-cache`), not a PATH shadow. Run `tools/proto/generate.sh` and commit the regenerated `gen/` and `turing-client/turing_app/lib/generated/` output. CI's `tools/proto/check.sh` compares generated bytes, so codegen must be deterministic.
