...
Recall deduplicates against the budget-admitted request, not the larger fetched history window. If adding recall changes the admitted history suffix, the runtime allows up to three convergence passes under one two-second deadline; one broad fallback then prefers a possible duplicate over silently losing a current-session turn from both history and recall.
