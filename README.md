...
Recall search runs once per agent run; cached hits are re-ranked against each dispatch's budget-admitted request rather than re-querying the unchanged terms across tool iterations. If adding recall changes the admitted history suffix, the runtime allows up to three ranking/budget passes under one two-second deadline; one broad fallback then prefers a possible duplicate over silently losing a current-session turn from both history and recall.
