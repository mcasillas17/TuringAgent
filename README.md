...
Recall deduplicates against the budget-admitted request, not the larger fetched history window. If adding recall changes the admitted history suffix, the runtime repeats that local recall/budget pass until the suffix is stable; a bounded fallback prefers a possible duplicate over silently losing a current-session turn from both history and recall.
